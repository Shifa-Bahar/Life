...
import com.lifepharmacy.application.ui.payment.WebViewPaymentActivityNew
...
//    binding.svMain.scrollTo(0, binding.svMain.getBottom());
  ...
  private fun placeOrderRequest(createOrderResponseModel: OrderResponseModel) {
      ...
      addressViewModel.deliveredAddressMut.value?.id?.let {
        ...
        when {
          viewModel.orderId == 0 -> {
            ...
            when (viewModel.selectedPaymentType.value?.name?.toLowerCase(Locale.ROOT)) {
              "wallet_with_less", "new_for_wallet" -> {
                ...
                isTopUp = false
                Log.e(TAG, "placeOrderRequest: newcard coming here")
                createOrder(placeOrderRequestOrder)
              }
            }
          }
          isTopUp -> {
            ...
            createTopUpTransaction(createOrderResponseModel)
          }
          else -> {
            ...
            when (viewModel.selectedPaymentType.value?.name?.toLowerCase(Locale.ROOT)) {
              "wallet_with_less", "new_for_wallet" -> {
                ...
                Log.e(TAG, "placeOrderRequest: 6")
                isTopUp = false
                createTransaction(createOrderResponseModel)
              }
            }
...
//                appManager.persistenceManager.order = it1
                handleCreateOrderResult(it1)
              }
            }
            Result.Status.ERROR -> {
              it.message?.let { it1 ->
  ...
  private fun handleCreateOrderResult(response: OrderResponseModel) {
    ...
    response.id?.let { orderId ->
      ...
      if (isTopUp) {
        ...
        createTopUpTransaction(response)
      } else {
  ...
  private fun createTopUpTransaction(createOrderResponseModel: OrderResponseModel) {
    isTopUp = true
//    cartViewModel.selectedPaymentType = topViewModel.selectedPaymentType
    topViewModel.selectedPaymentType = viewModel.selectedPaymentType
    topViewModel.token = viewModel.token
    topViewModel.isSavecard = viewModel.isSavecard

    topViewModel.amount.setValue(viewModel.walletDifference.value?.toString())

    Log.e(TAG, "createTopUpTransaction: ${topViewModel.selectedPaymentType}")
      ...
      .observe(viewLifecycleOwner) {
        it?.let {
          when (it.status) {
            Result.Status.SUCCESS -> {
              hideLoading()
              Log.e(TAG, "createTopUpTransaction: 1")
              Log.e(TAG, "createTopUpTransaction: ${topViewModel.selectedPaymentType}")
              topViewModel.selectedPaymentType = viewModel.selectedPaymentType
              if (it.data?.data?.redirect == true) {
                val makeURl = it.data?.data?.redirect_url.toString()
                openActivityForPaymentNew(makeURl)
//                  WebViewActivity.open(requireActivity(), makeURl, getString(R.string.lifeurl))
              } else {
                it.data?.data?.let { it1 -> handleTopUpTransaction(it1, createOrderResponseModel) }
              }
//              it.data?.data?.let { it1 -> handleTopUpTransaction(it1, createOrderResponseModel) }
            }
            Result.Status.ERROR -> {
              Log.e(TAG, "createTopUpTransaction: error")
              hideLoading()
              it.message?.let { it1 ->
                AlertManager.showErrorMessage(
                  requireActivity(),
              ...
              triggerMultiplecartEvent("checkout_failed")
  ...
  private fun triggerMultiplecartEvent(action: String) {
    appManager.offersManagers.getAllProductsWithQTY().let {
      Log.e(TAG, "triggerMultiplecartEvent: $it")
  ...
  private fun createTransaction(createOrderResponseModel: OrderResponseModel) {
    Log.e(TAG, "createTransaction: ${viewModel.selectedPaymentType}${viewModel.card_pg}")
    viewModel.createTransaction(viewModel.getTransactionModer(walletVieModel.selectedCard?.id))
      .observe(
        ...
        it?.let {
          when (it.status) {
            Result.Status.SUCCESS -> {
              Log.d(TAG, "createTransaction() returned: ${it.data?.data?.pg}")
              hideLoading()
              it.data?.let { it1 ->
                if (it1.data?.redirect == true) {
                  val makeURl = it1.data?.redirect_url.toString()
                  openActivityForPaymentNew(makeURl)
//                  WebViewActivity.open(requireActivity(), makeURl, getString(R.string.lifeurl))
                } else {
                  handleCreateTransactionForOrder(it1, createOrderResponseModel)
                }
              }
            }
            Result.Status.ERROR -> {
              Log.e(TAG, "createTransaction: ")
              it.message?.let { it1 ->
                AlertManager.showErrorMessage(
                  requireActivity(),
                  it1
...
//              viewModel.appManager.analyticsManagers.transactionFailed()
  ...
  private fun handleTopUpTransaction(
    result: TransactionModel,
    createOrderResponseModel: OrderResponseModel
  ) {
    isTopUp = false
    Log.e(TAG, "handleTopUpTransaction: ${viewModel.selectedPaymentType.value}")
...
//            openActivityForPayment(it1)
          ...
          if (result?.status == 0) {
            ...
            placeOrderRequest(createOrderResponseModel)
          }
        }
      }
      PaymentType.WALLET -> {
        Log.e(TAG, "handleTopUpTransaction: wallet")
//        viewModel.selectedPaymentType.value = PaymentType.WALLET
//        placeOrderRequest(createOrderResponseModel)
      }
      PaymentType.WALLET_WITH_LESS -> {
        Log.e(TAG, "handleTopUpTransaction: wallet with less")
        if (result?.status == 0) {
          AlertManager.showErrorMessage(
            requireActivity(),
            getString(R.string.transaction_failed)
  ...
  private fun handleCreateTransactionForOrder(
    result: GeneralResponseModel<TransactionModel>,
    createOrderResponseModel: OrderResponseModel
  ) {
    result?.data?.let {
      when (viewModel.selectedPaymentType.value) {
        PaymentType.CASH -> {
          orderPlace(result.data, createOrderResponseModel)
        }
        PaymentType.CARD -> {
          if (it.status == 0) {
            AlertManager.showErrorMessage(
...
//            it.urls?.let { it1 -> openActivityForPayment(it1) }
  ...
  private fun orderPlace(
    ...
    transactionModel?.let {
      viewModel.appManager.analyticsManagers.orderCompleted(
        it,
        createOrderResponseModel
      )
    }
    triggerMultiplecartEvent("checkout_completed")
...
//    WebViewPaymentActivity.open(requireActivity(), urls)
  ...
  private fun openActivityForPaymentNew(urls: String) {
    val intent = Intent(requireActivity(), WebViewPaymentActivityNew::class.java)
    intent.putExtra("paymentURL", urls)
    intent.putExtra("successURL", urls)
    intent.putExtra("failURL", urls)
    startActivityForResult(intent, ConstantsUtil.PAYMENT_ACTIVITY_REQUEST_CODE)
//    WebViewPaymentActivity.open(requireActivity(), urls)
  }
        ...
        ConstantsUtil.PAYMENT_ACTIVITY_REQUEST_CODE -> {
          ...
          if (status == 1) {
            ...
            when (viewModel.selectedPaymentType.value?.name?.toLowerCase(Locale.ROOT)) {
              "wallet_with_less", "new_for_wallet" -> {
                viewModel.selectedPaymentType.value = PaymentType.WALLET
                createTransaction(OrderResponseModel())
              }
              else -> {
                orderPlace(null, OrderResponseModel())
              }
            }
          } else {
    ...
    animator?.addListener(object : AnimatorListenerAdapter() {
      override fun onAnimationEnd(animation: Animator?) {
        ...
        if (!isAnimationCanceled) {
          placeOrderRequest(OrderResponseModel())
        }
      }
    })
    animator?.start()
  ...
  override fun onClickProceed() {
    triggerMultiplecartEvent("checkout_initiated")
    Log.e(TAG, "onClickProceed: ${viewModel.selectedPaymentType.value}")
    when (viewModel.selectedPaymentType.value) {
      PaymentType.CARD -> {
        animateProgressBar()
  ...
  override fun onClickCancel() {
    cancelAnimation()
    appManager.persistenceManager.ordercancel = true
