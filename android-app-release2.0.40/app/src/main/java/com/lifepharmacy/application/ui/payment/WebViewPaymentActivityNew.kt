...
@AndroidEntryPoint
class WebViewPaymentActivityNew : AppCompatActivity(), ClickTool {
  ...
  companion object {
    fun open(activity: Activity, urls: String) {
      val intent = Intent(activity, WebViewPaymentActivityNew::class.java)
      intent.putExtra("paymentURL", urls)
      intent.putExtra("successURL", "cko-payment/successful")
      intent.putExtra("failURL", "cko-payment/failed")
...
//          https://devapp.lifepharmacy.com/cko-payment/successful?cko-session-id=sid_zvzlun2txyxeba55b23kyz2ara
          Log.e(TAG, "doUpdateVisitedHistory: $url")
          var a = url?.contains("successful")
          var b = url?.contains("failed")
          if (a == true) {
            sendResultBack(1)
          } else if (b == true) {
            AlertManager.showErrorMessage(
              this@WebViewPaymentActivityNew,
